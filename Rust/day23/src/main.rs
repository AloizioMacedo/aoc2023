...
fn bfs_no_slope(
    ...
    let mut greatest_path_length = 0;
    let mut previous_grt_path_length = 0;

    while let Some(((i, j), p)) = queue.pop_back() {
        if i > 0 && !p.contains(&(i - 1, j)) && matrix[(i - 1, j)] != '#' {
            if (i - 1, j) == destination {
                greatest_path_length = greatest_path_length.max(p.len());
            } else {
                let mut p = p.clone();
                p.insert((i - 1, j));
                queue.push_back(((i - 1, j), p));
            }
        }
        if i < nrows - 1 && !p.contains(&(i + 1, j)) && matrix[(i + 1, j)] != '#' {
            if (i + 1, j) == destination {
                greatest_path_length = greatest_path_length.max(p.len());
            } else {
                let mut p = p.clone();
        ...
        if j > 0 && !p.contains(&(i, j - 1)) && matrix[(i, j - 1)] != '#' {
            if (i, j - 1) == destination {
                greatest_path_length = greatest_path_length.max(p.len());
            } else {
                let mut p = p.clone();
                p.insert((i, j - 1));
                queue.push_back(((i, j - 1), p));
            }
        }
        if j < ncols - 1 && !p.contains(&(i, j + 1)) && matrix[(i, j + 1)] != '#' {
            if (i, j + 1) == destination {
                greatest_path_length = greatest_path_length.max(p.len());
            } else {
                let mut p = p.clone();
        ...
        if previous_grt_path_length < greatest_path_length {
            println!("{}", greatest_path_length);
            previous_grt_path_length = greatest_path_length;
        }
    }
...
fn solve_part_two(contents: &str) -> Result<usize> {
    ...
    Ok(bfs_no_slope(&matrix, (0, 1), (nrows - 1, ncols - 2)))
}
...
mod tests {
    ...
    #[test]
    fn part_two() {
        assert_eq!(solve_part_two(TEST).unwrap(), 154);
    }
}
