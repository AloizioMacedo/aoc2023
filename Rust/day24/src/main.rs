...
use crate::hailstone::{get_shift_velocity, is_point_in_past, Position, Vector};
...
fn solve_part_two(contents: &str) -> Result<f64> {
    ...
    let intersection = get_intersection_at_xy(
        Hailstone {
            position: position1,
            velocity: Vector {
                x: vx1 - velocity_to_shift[0],
                y: vy1 - velocity_to_shift[1],
                z: vz1 - velocity_to_shift[2],
            },
        },
        Hailstone {
            position: position2,
            velocity: Vector {
                x: vx2 - velocity_to_shift[0],
                y: vy2 - velocity_to_shift[1],
                z: vz2 - velocity_to_shift[2],
            },
        },
    );
